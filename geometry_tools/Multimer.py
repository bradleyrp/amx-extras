...
#---COPIED!
def martinize(pdb=None):
	"""
	Use martinize to generate a coarse-grained protein.
	"""
	#---! trying some new hookups for multimer
	#if not state: state = globals()['state']
	name = 'protein'
	...
	cmd += ' -f %s -o %s.top -x %s.pdb '%(pdb if pdb else 'protein-start.pdb',name,name)
	...
	#---we wish to carefully control the position restraints for fixing the bilayer so we cleave 
	#---...them from the ITP previously this was done with bash and 
	#---..."sed -i 's/POSRES_FC    POSRES_FC    POSRES_FC/0 0 POSRES_FC/g"
...
class ProteinMultimer:
	...
	def martinize(self,**kwargs):
		...
		Wrap martinize.
		"""
		gmx('editconf',structure=kwargs['gro'],
			o='%s.pdb'%kwargs['gro'],log='editconf-martinize-%s'%kwargs['gro'])
		martinize(pdb='%s.pdb'%kwargs['gro'])
		#---! fix the name protein.gro
		self.contents[kwargs['name']] = GMXStructure(state.here+'protein.gro')
